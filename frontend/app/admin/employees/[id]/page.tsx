...
export default function EmployeeDetailPage() {
      ...
      .then(data => {
        if (data.success && data.data) {
          ...
          setCredentials(data.credentials || {
            ...
            email: data.data.email || 'employee@spysalon.com',
            username: data.data.username || 'employee',
            tempPassword: data.data.tempPassword || '●●●●●●●● (Sent via Email)'
          });
        }
      })
      .catch(console.error)
  ...
  const handleCopyCredentials = () => {
    ...
    const empEmail = credentials?.email || employee?.email || 'employee@spysalon.com';
    const empPassword = credentials?.tempPassword || employee?.tempPassword || '●●●●●●●● (Sent via Email)';
  ...
  const loginEmail = credentials?.email || employee.email || 'employee@spysalon.com';
  const loginPassword = credentials?.tempPassword || employee.tempPassword || '●●●●●●●● (Sent via Email)';
      ...
      <div className="max-w-4xl mx-auto space-y-8 animate-fadeIn text-left">
        ...
        <div className="glass-card p-6 sm:p-8 rounded-3xl border border-rosegold-500/30 flex flex-col md:flex-row items-center md:items-start gap-6">
          <div className="flex flex-col items-center space-y-3 shrink-0">
            <div className="w-28 h-28 sm:w-32 sm:h-32 rounded-3xl overflow-hidden border-2 border-rosegold-500/50 shadow-glow-rosegold bg-dark-800 flex items-center justify-center brand-profile-avatar">
              {employee.avatar && !employee.avatar.includes('photo-1534528741775') ? (
                <img
                  src={employee.avatar}
                  alt={employee.name}
                ...
                <span className="font-extrabold text-rosegold-400 text-2xl">
                  {employee.name ? employee.name.split(' ').map((n: string) => n[0]).join('').toUpperCase().slice(0, 2) : 'ST'}
                </span>
              )}
            </div>
