...
import { validateForm, validateName, validateEmail, validatePhone, validateRequired } from '@/lib/validation';
...
export default function ContactPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isValid) {
      ...
      setErrorMsg(firstErr || 'Please fix errors before submitting your inquiry.');
      return;
    }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        >
          ...
          {submitted && submittedData ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="text-xs text-rosegold-300 font-bold uppercase tracking-wider block mb-1">Your Name *</label>
                <input
                  type="text"
                  required
                  ...
                  className="w-full p-3 rounded-xl bg-dark-800 border border-white/10 text-white text-sm focus:outline-none focus:border-rosegold-500 font-medium transition-colors disabled:opacity-50"
                />
                {fieldErrors.name && <p className="text-red-400 text-xs font-bold pt-0.5">{fieldErrors.name}</p>}
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="text-xs text-rosegold-300 font-bold uppercase tracking-wider block mb-1">Email *</label>
                  <input
                    type="email"
                    required
                    disabled={isSubmitting}
                    ...
                    className="w-full p-3 rounded-xl bg-dark-800 border border-white/10 text-white text-sm focus:outline-none focus:border-rosegold-500 font-medium transition-colors disabled:opacity-50"
                  />
                  {fieldErrors.email && <p className="text-red-400 text-xs font-bold pt-0.5">{fieldErrors.email}</p>}
                </div>
                <div>
                  <label className="text-xs text-rosegold-300 font-bold uppercase tracking-wider block mb-1">Phone *</label>
                  <input
                    type="tel"
                    required
                    disabled={isSubmitting}
                    ...
                    className="w-full p-3 rounded-xl bg-dark-800 border border-white/10 text-white text-sm focus:outline-none focus:border-rosegold-500 font-medium transition-colors disabled:opacity-50"
                  />
                  {fieldErrors.phone && <p className="text-red-400 text-xs font-bold pt-0.5">{fieldErrors.phone}</p>}
                </div>
              </div>
              ...
              <div>
                <label className="text-xs text-rosegold-300 font-bold uppercase tracking-wider block mb-1">Message *</label>
                <textarea
                  rows={4}
                  required
                  disabled={isSubmitting}
                  ...
                  className="w-full p-3 rounded-xl bg-dark-800 border border-white/10 text-white text-sm focus:outline-none focus:border-rosegold-500 resize-none font-medium transition-colors disabled:opacity-50"
                />
                {fieldErrors.message && <p className="text-red-400 text-xs font-bold pt-0.5">{fieldErrors.message}</p>}
              </div>
        ...
        >
          <h2 className="text-2xl sm:text-3xl font-bold font-serif text-white tracking-wide flex items-center space-x-2">
            <MapPin className="w-6 h-6 text-rosegold-400 shrink-0" />
          ...
          {[
            ...
            <motion.div 
              ...
              className="glass-card p-5 rounded-2xl space-y-2.5 border border-rosegold-500/30 hover:border-rosegold-400 bg-dark-800/90 transition-all cursor-pointer shadow-xl group"
            >
              <h3 className="text-rosegold-300 group-hover:text-white font-serif font-bold text-xl tracking-wide transition-colors">{branch.title}</h3>
              <p className="text-xs text-gray-300 font-medium leading-relaxed">{branch.addr}</p>
              <div className="flex items-center space-x-1.5 text-xs text-rosegold-400 font-bold pt-1">
