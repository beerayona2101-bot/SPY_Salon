...
import { Search, Star, Clock, Sparkles, ArrowRight, Eye, CheckCircle2, Crown, Award, ShieldCheck, Filter } from 'lucide-react';
...
function ServicesContent() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-12">
      ...
      {/* Category Filter & Search Ribbon (Clean Filter Option - No Horizontal Scroll buttons) */}
      <div className="p-4 sm:p-5 rounded-3xl bg-dark-850 border border-white/10 shadow-xl space-y-4">
        <div className="flex flex-col md:flex-row items-center justify-between gap-4">
          
          {/* Category Dropdown Filter Option */}
          <div className="w-full md:w-auto flex items-center space-x-2.5">
            ...
            <select
              ...
              className="w-full md:w-auto bg-dark-800 text-rosegold-300 font-bold text-xs px-4 py-2 rounded-full border border-rosegold-500/40 focus:outline-none focus:border-rosegold-400 cursor-pointer shadow-sm truncate"
            >
              {categories.map((cat) => (
                <option key={cat} value={cat} className="bg-dark-900 text-white">
                  {cat === 'All' ? '✨ All Treatment Categories' : cat}
                </option>
              ))}
            </select>
          </div>

          {/* Wrapped Category Filter Badges for Large Screens */}
