...
export default function ServiceDetailPage() {
  ...
  useEffect(() => {
    ...
    const loadSingleService = async () => {
      try {
        // 1. Query direct single service endpoint
        const resSingle = await fetch(`${API_BASE_URL}/services/${encodeURIComponent(id)}`);
        const jsonSingle = await resSingle.json();
        if (resSingle.ok && jsonSingle.success && jsonSingle.data) {
          const match = jsonSingle.data;
          setService({
        ...
        // 2. Query full services list if direct lookup returns 404
        const resAll = await fetch(`${API_BASE_URL}/services`);
