...
import { PricingSkeleton } from '@/components/common/Skeleton';
...
function PricingPageContent() {
  ...
  // 1. Build services list EXCLUSIVELY from MongoDB liveDbServices
  const allServices: ServiceItem[] = useMemo(() => {
    if (!liveDbServices || liveDbServices.length === 0) {
      return [];
    }

    return liveDbServices.map((dbSrv: any) => ({
      id: dbSrv._id || dbSrv.id,
      name: dbSrv.name,
      category: dbSrv.category || 'Hair Care',
      subCategory: dbSrv.subCategory || dbSrv.category?.toLowerCase().replace(/[^a-z0-9]+/g, '-') || 'haircare',
  ...
  // 2. Build Subcategory Structure for CategorySidebar dynamically purely from MongoDB liveDbServices
  const genderCategoryGroups: GenderCategoryGroup[] = useMemo(() => {
    const genders: Array<{ id: 'men' | 'women' | 'kids'; title: string; icon: string }> = [
      { id: 'men', title: "Men's Salon", icon: '👨' },
      { id: 'women', title: "Women's Salon", icon: '👩' },
      { id: 'kids', title: "Kids' Salon", icon: '🧒' }
    ];
    ...
    return genders.map((g) => {
      ...
      if (allServices && allServices.length > 0) {
        ...
        sectionServices.forEach(srv => {
          ...
          const slug = srv.subCategory || catName.toLowerCase().replace(/[^a-z0-9]+/g, '-');
          const existing = subcategoryMap.get(slug);
          if (existing) {
            existing.count += 1;
          } else {
            subcategoryMap.set(slug, { name: catName, slug, count: 1 });
          }
        });
      }
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-10 space-y-8 text-left">
          ...
          {selectedService ? (
            ...
            <div className="p-12 text-center glass-card rounded-3xl border border-white/10 space-y-4">
              ...
              <h3 className="text-xl font-serif font-bold text-white">
                {allServices.length === 0 ? 'No Services in Catalogue Yet' : 'No Service Selected'}
              </h3>
      ...
      <motion.div
        ...
        transition={{ duration: 0.3 }}
        className="p-8 sm:p-10 rounded-3xl border border-amber-600/40 bg-gradient-to-r from-amber-800 via-amber-900 to-dark-900 text-white flex flex-col sm:flex-row items-center justify-between gap-6 shadow-2xl mt-12 force-promo-card"
      >
        <div className="space-y-2 text-center sm:text-left">
          <div className="inline-flex items-center space-x-1.5 px-3.5 py-1 rounded-full bg-black/50 text-amber-200 border border-amber-400/30 text-[10px] font-extrabold uppercase tracking-wider mb-1">
            <Sparkles className="w-3.5 h-3.5 text-amber-300" />
            <span className="force-text-white">SPECIAL PROMO</span>
          </div>
          <h3 className="text-2xl sm:text-3xl font-serif font-extrabold text-white tracking-wide force-text-white">
            ✨ 25% OFF FOR ALL NEW CUSTOMERS
          </h3>
          <p className="text-xs sm:text-sm font-medium text-amber-100/90 leading-relaxed force-text-white max-w-xl">
        ...
        <Link
          href="/book"
          className="px-8 py-4 rounded-full bg-white hover:bg-amber-100 text-dark-900 font-serif font-extrabold text-sm shadow-2xl hover:scale-105 transition-all cursor-pointer shrink-0 inline-flex items-center space-x-2 border border-amber-200"
        >
          <span className="text-dark-900 font-bold">Book Appointment Now</span>
