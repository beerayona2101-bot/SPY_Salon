      ...
      try {
        ...
        if (data.success && data.data && Array.isArray(data.data) && data.data.length > 0) {
          const mapped = data.data.map((emp: any, idx: number) => {
            const exps = ['12+ Years Exp', '9+ Years Exp', '10+ Years Exp', '7+ Years Exp'];
            return {
              name: emp.name,
              title: emp.specialties?.join(' & ') || 'Specialist',
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-16">
      ...
      <div className="space-y-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {team.map((member) => (
            <div key={member.name} className="glass-card rounded-2xl overflow-hidden text-center space-y-3 pb-5 flex flex-col justify-between">
              <div>
                {member.img ? (
                  <img src={member.img} alt={member.name} className="w-full h-56 object-cover" />
                ) : (
                  <div className="w-full h-56 bg-dark-800 flex items-center justify-center text-rosegold-400 text-3xl font-bold font-serif brand-profile-avatar">
                    {member.name ? member.name.split(' ').map((n: string) => n[0]).join('').toUpperCase().slice(0, 2) : 'SP'}
                  </div>
                )}
