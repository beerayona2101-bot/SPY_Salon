...
export default function EmployeeCalendarModule({
  ...
  const [currentDate, setCurrentDate] = useState<Date>(new Date(2026, 6, 28)); // Default July 2026
  const [viewMode, setViewMode] = useState<'Month' | 'Week' | 'Day' | 'Agenda'>('Month');
  const [selectedDayDate, setSelectedDayDate] = useState<string | null>('2026-07-28');
  ...
  const [leaveForm, setLeaveForm] = useState({ startDate: '2026-08-05', endDate: '2026-08-06', reason: '' });
  ...
  const getDayAppointmentsAndRevenue = (dateStr: string) => {
    ...
    const revenue = completedApps.reduce((sum, a) => {
      ...
      if (a.service?.includes('Facial')) return sum + 1499;
                ...
                >
                  ...
                  {isCurrentMonth && (
                    <div className="space-y-1 text-[10px]">
                      {dayMetrics.count > 0 && (
                        <div className="px-1.5 py-0.5 rounded-md bg-dark-900/80 text-rosegold-300 font-bold truncate">
                          {dayMetrics.count} Appts (₹{dayMetrics.revenue})
                        </div>
                      )}
                      ...
                      {dayMetrics.completed.length > 0 && (
                        <div className="flex items-center space-x-0.5 text-yellow-400 font-mono">
                    ...
                    {slot.isBreak ? (
                      ...
                      <div className="p-3 rounded-2xl bg-dark-850 border border-rosegold-500/40 flex items-center justify-between text-xs hover:border-rosegold-400 transition-colors">
                        ...
                        <div className="flex items-center space-x-3">
                          ...
                          <span className="font-serif font-bold text-rosegold-400">₹{appMatch.price || 1499}</span>
                        </div>
                      </div>
                    ) : (
            ...
            {appointments.map(app => (
              <div key={app._id} className="p-4 rounded-2xl bg-dark-850 border border-white/10 flex flex-col sm:flex-row sm:items-center justify-between gap-3 hover:border-rosegold-500/40 transition-colors">
                ...
                <div className="flex items-center space-x-4">
                  ...
                  <span className="font-serif font-bold text-rosegold-400 text-sm">₹{app.price || 1499}</span>
                </div>
              </div>
            ))}
          </div>
              ...
              {selectedDayInfo.metrics.all.map(app => (
                <div key={app._id} className="p-3.5 rounded-2xl bg-dark-850 border border-white/10 flex items-center justify-between text-xs">
                  ...
                  <div className="text-right space-y-1">
                    <span className="font-serif font-bold text-rosegold-400 text-sm block">₹{app.price || 1499}</span>
